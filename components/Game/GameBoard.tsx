            ...
            <div className={`${showControls ? 'max-w-6xl' : 'max-w-3xl'} mx-auto px-4 py-8 transition-all duration-300`}>
                <div className={`grid gap-8 transition-all duration-300 ${showControls ? 'lg:grid-cols-3' : 'lg:grid-cols-1'}`}>
                    ...
                    <div className={`transition-all duration-300 ${showControls ? 'lg:col-span-2' : 'lg:col-span-1'}`}>
                        ...
                        {/* Mobile Quick Controls - Only show when sidebar is hidden on large screens */}
                        {!showControls && (
                            <div className="hidden lg:flex justify-center gap-4 mb-6">
                                <button
                                    onClick={handleClearPath}
                                    disabled={gameState.isGameComplete || !!completedPath}
                                    className={`px-4 py-2 rounded-lg transition-colors font-medium shadow-md ${gameState.isGameComplete || completedPath
                        ...
                        {/* Related Games Section - Inside the game area */}
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            <h2 className="text-xl font-bold text-gray-800 mb-4 text-center">🎮 More Games You&apos;ll Love</h2>
                    ...
                    {/* Control Panel - Collapsible on large screens, always visible on mobile */}
                    <div className={`transition-all duration-300 ${showControls ? 'lg:col-span-1 lg:opacity-100 lg:max-w-none lg:block' : 'lg:col-span-0 lg:opacity-0 lg:max-w-0 lg:hidden'
                        } block opacity-100`}>
                        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
                            <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="space-y-3">
                                <button
                                    onClick={handleClearPath}
                                    disabled={gameState.isGameComplete || !!completedPath}
                                    className={`w-full px-4 py-3 rounded-lg transition-colors font-medium shadow-md ${gameState.isGameComplete || completedPath
                                        ? 'bg-gray-400 cursor-not-allowed text-gray-200'
                                        : 'bg-yellow-500 hover:bg-yellow-600 text-white'
                                        }`}
                                >
                        ...
                        <div className="flex border-b bg-gray-50">
                            {helpSections.map((section) => (
                                <button
                                    ...
                                    className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${activeHelpSection === section.id
